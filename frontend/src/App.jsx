...
function App() {
  ...
  const loadCandies = async () => {
    try {
      ...
      showMessage('error', 'Kon snoepjes niet laden: ' + error.message);
    }
  };
  ...
  const handleCheckout = async (e) => {
    ...
    try {
      ...
      // Add customer if new
      if (!customers.find(c => c.email === customerInfo.email)) {
        setCustomers([{
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      showMessage('error', error.message || 'Bericht verzenden mislukt');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="dashboard-app">
      <header className="dashboard-header">
        <h1>🍬 Snoepjes Winkel Dashboard 🍬</h1>
        <p>Beheer alles vanuit één centrale locatie</p>
      </header>
